...
Standalone, IC-grade macro tracker. Six tabs: **Overview** (regime + Fed read-through + surprise scorecard + release calendar), **Liquidity** (the Guide to the Market mirror — 522-week net liquidity vs S&P 500, supportive/uncomfortable/stress zones, SOFR-IORB), **CPI** (with the BLS Table A breakdown), **PPI** (final-demand Table A + intermediate-demand pipeline pressure), **Jobs**, **PCE**. Single-file HTML, Bloomberg-dark, EN/繁 toggle, SHA-256 access gate. Data is **baked from FRED at build time** by a scheduled GitHub Action — the deployed page stays static, so there is no exposed API key and no client-side CORS.
...
**CPI Table A line items** (SA + CUUR-prefixed NSA pairs) — CUSR0000SAF11 (food at home), CUSR0000SEFV (food away), CUSR0000SACE (energy commodities), CUSR0000SETB01 (gasoline), CUSR0000SEHE (fuel oil & other fuels), CUSR0000SEHF (energy services), CUSR0000SEHF01 (electricity), CUSR0000SEHF02 (piped gas), CUSR0000SETA01 (new vehicles), CUSR0000SETA02 (used cars), CPIAPPSL/CPIAPPNS (apparel), CUSR0000SAM1 (medical commodities), CUSR0000SAS4 (transportation services), CUSR0000SAM2 (medical services). The CPI tab renders these as the BLS Table A hierarchy (3× SA MoM + NSA YoY; NSA-only items shown unadjusted, per BLS practice).
**PPI (FD-ID)** — final demand PPIFIS/PPIFID; goods PPIDGS/PPIFDG; foods PPIDFS/PPIFDF; energy PPIDES/PPIFDE; goods less foods & energy WPSFD413/WPUFD413; services PPIDSS/PPIFDS; trade services PPITSS/PPIDTS; transportation & warehousing PPIAWS/PPITAW; services less trade/transp/whse PPITWS/PPITTW; construction PPIDCS/PPIFDC; less foods & energy PPIFES/PPICOR; less foods, energy & trade WPSFD49116/WPUFD49116; intermediate demand WPSID61/62/63 (+WPU NSA twins). SA for MoM, NSA for 12-month, per the BLS PPI release convention. PPI revises up to 4 months after first publication, so recent monthly cells can shift between bakes.
